            ...
            for y in range(img.shape[0]):
                for x in range(img.shape[1]):
                    for channel in range(img.shape[2]):
                        new_img_compressed[y, x, channel] = np.clip(
                            alpha * img[y, x, channel] + beta, min, max
                        )
        case 1:
            for y in range(img.shape[0]):
                for x in range(img.shape[1]):
                    new_img_compressed[y, x] = np.clip(
                        alpha * img[y, x] + beta, min, max
                    )
            ...
            for y in range(img.shape[0]):
                for x in range(img.shape[1]):
                    for c in range(img.shape[2]):
                        new_img_gammaCorr[y, x, c] = (
                            255 * coef_c * (img[y, x, c] / 255) ** coef_gamma
                        )
        case 1:
            for y in range(img.shape[0]):
                for x in range(img.shape[1]):
                    new_img_gammaCorr[y, x] = (
                        255 * coef_c * (img[y, x] / 255) ** coef_gamma
                    )
    ...
    match flag:
        case 0:
            for y in range(img.shape[0]):
                for x in range(img.shape[1]):
                    for c in range(img.shape[2]):
                        new_img_logCorr[y, x, c] = (
                            255 * coef_c * math.log((img[y, x, c] / 255) + 1)
                        )
        case 1:
            for y in range(img.shape[0]):
                for x in range(img.shape[1]):
                    new_img_logCorr[y, x] = (
                        255 * coef_c * math.log((img[y, x] / 255) + 1)
                    )
    ...
    match flag:
        case 0:
            ...
            for y in range(img.shape[0]):
                for x in range(img.shape[1]):
                    for channel in range(img.shape[2]):
                        new_img_linCorr[y, x, channel] = (
                            (img[y, x, channel] - list_min[channel])
                            * 255
                            / (list_max[channel] - list_min[channel])
                        )
        case 1:
